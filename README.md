...
- **Database Design**: 5-layer architecture on Supabase PostgreSQL with raw landing, staging, intermediate logic, analytics marts, and isolated pgvector HNSW indexing
- **Orchestration**: Dagster pipeline with software-defined assets, native dbt integration, and full end-to-end data lineage tracking
- **RAG Pipeline**: Zero-dependency implementation with local vector embeddings, semantic similarity search, and streaming natural language responses
- **Context Rewriting**: Pre-search query contextualization that correctly resolves pronouns and maintains state across conversation history
- **SvelteKit Frontend**: Complete dashboard with tournament brackets, standings, game browser, player leaderboards, profiles, and integrated AI chat
- **Production Deployment**: Dockerized pipeline hosted on AWS EC2, SvelteKit frontend on Vercel, full CI/CD with automated data validation testing
